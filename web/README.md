...
- 词表包含 1,911 个内置单词。进入单词速记时先设定本轮目标，默认 30 个；本轮单词固定，标记掌握后不会自动补入新词。
- 切换顺序、随机或具体首字母前会先确认重新生成；取消可保留当前批次。已掌握单词可单独浏览，未完成批次不会跨页面保存；已掌握状态只保存在当前浏览器。
- 当前批次的发音提前读取；连续点击发音时以最后选择的单词为准，并显示加载或失败状态。
...
- `深大学位英语题库_公开版_v1.2.0.zip`：可上传到 GitHub Release 的无密码通用下载包。
