...
- 成品需要适配电脑、手机和平板；一期交付边界为同一可信局域网内的本地浏览器学习版：电脑临时提供服务，平板通过局域网地址访问。用户确认暂不需要公网发布、登录或跨设备同步，因此这些能力不能宣称已完成，也不作为本期阻塞项。
...
16. `cabc768 docs: record additional content audit`
17. `81e1705 content: refine cloze expression`
...
- 最新人工审校记录见 `web/docs/content-audit.md`：已完整审读 16 篇写作范文、首次诊断 20 题、基础语法与高频易错语法共 360 题、12 篇完形全文、阅读理解 16 篇 / 64 题及汉译英 80 句；已抽样审读完形 36 空。写作修正两处搭配，完形 `cloze-035` 修正为更自然的 `improved by taking small steps`；汉译英 `translation-026`、`translation-063` 与语法 `grammar-019`、`grammar-057`、`grammar-105`、`grammar-301` 亦完成自然表达、唯一性或易错提示的修正，并已补回归断言。更新后的受影响内容仍需复核。
...
1. **内容审校与报告**：继续按 `web/docs/content-audit.md` 的待审范围复核完形填空尚未审读的 204 空，以及题目更新时的受影响内容。首次诊断 20 题、16 篇写作、基础语法与高频易错语法共 360 题、阅读 16 篇/64 题及汉译英 80 句已完成本轮人工审校；完形已通读 12 篇全文并审读 36 空。
2. **专题页细化**：根据每个专题的实际分层显示“基础”或“基础至冲刺”；之后可加入薄弱专题推荐和主观题草稿完成提示。
3. **局域网验收**：在电脑运行 `npm run build` 和 `npm run serve:lan`，平板通过 `http://电脑局域网IPv4:4173` 访问；实测进入练习、保存草稿和刷新恢复。局域网 HTTP 不保证 PWA 安装或离线缓存，且各设备 LocalStorage 不同步。
4. **远期扩展（非本期）**：如后续需要公网发布或跨设备同步，再由用户提供部署渠道及 Supabase 项目配置，接入登录、远端事件同步和 RLS。
...
- 不要声明“已跨设备同步”或“已公网发布”。一期交付是本地局域网浏览器版；各设备学习数据独立保存在各自浏览器中。

## 下一轮的精确续接点

1. 根据 `web/docs/content-audit.md` 对待复核模块进行人工语言审校；每次只记录实际审读过的题号和结果。下一步从完形填空尚未审读的 204 空开始。
2. Playwright 的两个用例已在系统 Microsoft Edge 152.0.4191.66 上通过：桌面端“主动核对后看解析”、手机端“保存后恢复汉译英草稿”。配置现在通过 `channel: 'msedge'` 复用系统浏览器，不依赖 Playwright 私有 Chromium 下载；移动端用例改为精确匹配“保存草稿”按钮，避免与“保存草稿并查看参考答案”发生歧义。
3. 本期局域网启动方式：用户日常使用时双击项目根目录的 `启动学位英语题库.cmd` 或其桌面快捷方式；启动器会执行构建、局域网服务和电脑浏览器打开。启动窗口中的 `Network` 地址供同一可信 Wi-Fi 下的平板访问；窗口关闭即停止服务。若 Windows 提示防火墙，仅允许专用网络。公网部署或跨设备同步均为后续可选项。
