...
3. 本期局域网启动方式：在 `web` 目录运行 `npm run build` 后运行 `npm run serve:lan`；查询电脑正在使用网络的 IPv4 地址，平板在同一可信 Wi-Fi 下访问 `http://IPv4:4173`。若 Windows 提示防火墙，仅允许专用网络。公网部署或跨设备同步均为后续可选项。
